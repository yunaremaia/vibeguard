...
DANGEROUS_PATTERNS = [
    # Python eval()
    (
        r"""(?<![\w])eval\s*\(\s*(?:request|req|input|params|body|data|user)""",
        "eval() called with user input — arbitrary code execution vulnerability",
    ),
    # Python exec()
    (
        r"""(?<![\w])exec\s*\(\s*(?:request|req|input|params|body|data|user)""",
        "exec() called with user input — arbitrary code execution vulnerability",
    ),
    # Python os.system()
    (
        r"""(?<![\w])os\.system\s*\(\s*(?:request|req|input|params|body|data|user)""",
        "os.system() called with user input — command injection vulnerability",
    ),
    # Python subprocess with shell=True
    (
        r"""(?<![\w])subprocess\.(?:run|call|Popen)\s*\(.*shell\s*=\s*True""",
        "subprocess with shell=True — command injection risk if user input reaches it",
    ),
    # JavaScript eval()
    (
        r"""(?<![\w])eval\s*\(\s*(?:req|request|params|query|body|user)""",
        "eval() called with user input — arbitrary code execution vulnerability",
    ),
    # JavaScript Function() constructor
    (
        r"""(?<![\w])new\s+Function\s*\(\s*(?:req|request|params|query|body|user)""",
